...
public class MinerDemo4  {
	...
	MinerDemo4(String server, int port,Miner m) {
		this.server = server;
		this.port = port;
		this.user=m;
		this.blockSize=0;
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		Miner m=new Miner();
		MinerDemo4 client = new MinerDemo4(serverAddress, portNumber, m);
				...
				try {
					...
					System.out.println(blockSize+"<"+user.getCurrentBlockSize());
						...
						if(blockSize==user.getCurrentBlockSize()){
							...
							String pow=user.getNearMiss(result,4);
							user.getBlock().setNearMiss(4);

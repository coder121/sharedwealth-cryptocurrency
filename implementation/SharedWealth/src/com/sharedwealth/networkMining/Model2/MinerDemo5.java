package com.sharedwealth.networkMining.Model2;
...
public class MinerDemo5  {
	...
	MinerDemo5(String server, int port,Miner m) {
		this.server = server;
		this.port = port;
		this.user=m;
		this.blockSize=0;
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		Miner m=new Miner();
		MinerDemo5 client = new MinerDemo5(serverAddress, portNumber, m);
				...
				try {
					...
					System.out.println(blockSize+"<"+user.getCurrentBlockSize());
						...
						if(blockSize==user.getCurrentBlockSize()){
							...
							String pow=user.getNearMiss(result,5);
							user.getBlock().setNearMiss(5);
					...
					if(o instanceof Block){
						display("Got the block from server");
					}

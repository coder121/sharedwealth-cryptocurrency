...
//						display("Miner Info:\n>"+m.toString());
						...
						for(User2 u:users){
							...
							if(!isHasRewarded()){
								...
								for(Miner m:miner){
									display(m.getId()+"==Actual:"+uId);
									if(m.getId()!=uId&&m.isHasSolvedPOW()){
//									  
//										m.setHasSolvedPOW(true);
//										display("Testing the solution"+m.getPwSolution());
//										m.setPwSolution(m.getPwSolution());
//										m.setBlockHash("abc");
										sOutput.reset();
										display("Sending the miner object after rewarding");
						...
						if(!isHasRewarded()){
						String minerInfo=(String)o;
						String [] solutionArray=minerInfo.split("\n");
						uId=Integer.parseInt(solutionArray[0].split(":")[1]);
						String [] powOfString=solutionArray[solutionArray.length-2].split(":");
						String blockHash=solutionArray[solutionArray.length-1].split(":")[1];
					    display("BlockHash:"+blockHash);
						
						boolean solvedPOW=ProofOfWork.Verify(Integer.parseInt(powOfString[1]), blockHash,powOfString[2]);
						if(solvedPOW){
							display("Rewarding the miner");
							setHasRewarded(true);
						}
					    solution=powOfString[1];
						display("Miner's id:"+uId+"\n BlockHash:"+blockHash+"\nNonce:"+powOfString[1]+"\nSolution:"+powOfString[2]);
					}
						else{
							display("Already rewarded");
							display("Miner's id:"+uId);
